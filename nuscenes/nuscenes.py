...
#from NuscenesDataset.nuscenes.lidarseg.lidarseg_utils import colormap_to_colors, plt_to_cv2, get_stats, \
...
class NuScenes:
    ...
    def __make_reverse_index__(self, verbose: bool) -> None:
        ...
        # Dictionary mapping scene names to their linear indices, update 210304
        self.scene_name_to_scene_idx = {scene['name']: idx for idx, scene in enumerate(self.scene)}
        self.scene_idx_to_scene_name = {idx: scene['name'] for idx, scene in enumerate(self.scene)}
...
class NuScenesExplorer:
    ...
    # update, 210305
    def get_annotations_with_attribute_names(self, sample, attribute_names):
        all_annotation_toks = sample['anns']
        matching_annotations = []
        for ann_tok in all_annotation_toks:
            annotation = self.nusc.get('sample_annotation', ann_tok)
            attr_tokens = annotation['attribute_tokens']
        ...
        return sorted(matching_annotations, key=lambda a: annotation['token'])

    # update, 210305
    def get_annotation_instance_token_track(self, instance_token):
        return self.nusc.field2token('sample_annotation', 'instance_token', instance_token)

    # update, 210305
    def get_annotation_track(self, instance_token):
        ann_inst_tokens = self.get_annotation_instance_token_track(instance_token)
        return [self.nusc.get('sample_annotation', ann_inst_token) for ann_inst_token in ann_inst_tokens]
...
# update, 210304
def traverse_linked_list(nusc, obj, tablekey='sample', direction='prev', inclusive=False):
    """

    :param nusc: NuScenes object
    :param obj: object token that's associated with a 'tablekey' sequence
    :param tablekey: name of the object's sequence1
    :param direction: which direction to traverse the list in.
    :param inclusive: whether to include 'object'
    :returns: an in-order traversal of the sequence starting from the object (exclusive)
    :rtype: list
    ...
    if not inclusive: data = []
    else: data = [cur]
    while True:
        if len(cur[direction]):
            cur = nusc.get(tablekey, cur[direction])
            data.append(cur)
        else:
            break
    if direction == 'prev':
...
# update, 210304
def interpolate_boxes_to_times(boxes, box_timestamps_relative, lidar_tokens, relative_times_interp):
    n_boxes = len(boxes)
    assert(n_boxes > 1)
    assert(n_boxes == len(box_timestamps_relative))
    assert(sorted(box_timestamps_relative) == box_timestamps_relative)

    # Ensure they're reasonable relative times (fails on unnormalized unix times).
    assert(max(box_timestamps_relative) <= 10000)
    assert(max(relative_times_interp) <= 10000)
    assert(min(box_timestamps_relative) <= min(relative_times_interp) < max(relative_times_interp) < max(box_timestamps_relative))

    interpolated = []
    lidar_tokens_sorted = []
    sort_indices = np.searchsorted(box_timestamps_relative, relative_times_interp)
    for interp_time, sort_index in zip(relative_times_interp, sort_indices):
        if sort_index == 0 or sort_index == n_boxes:
            raise IndexError("Can't interpolate out of time range!")
        else:
            boxleft = boxes[sort_index - 1]
            boxright = boxes[sort_index]
            timeleft = box_timestamps_relative[sort_index - 1]
            timeright = box_timestamps_relative[sort_index]
            interpolated.append(interpolate_box(boxleft, boxright, timeleft, timeright, interp_time))

            best_match_index = np.argmin(np.abs(np.array(box_timestamps_relative) - interp_time))
            lidar_tokens_sorted.append(lidar_tokens[best_match_index])
    return interpolated, lidar_tokens_sorted

# update, 210304
def interpolate_box(box0, box1, t0, t1, t):
    assert(t0 <= t <= t1)
    if t == t0: return box0
    if t == t1: return box1

    # Interpolate center.
    center = [np.interp(t, [t0, t1], [c0, c1]) for c0, c1 in zip(box0.center, box1.center)]

    # Interpolate orientation.
    rotation = Quaternion.slerp(q0=box0.orientation, q1=box1.orientation, amount=(t - t0) / (t1 - t0))

    return Box(center, box0.wlh, rotation, name=box0.name)