...
class PointCloud(ABC):
    ...
    def from_file_multisweep(cls,
        ...
        if sample_data_token is None:
            ...
            ref_sd_token = sample_data_token
            assert (ref_chan == chan)
...
class EgoBox(Box):
    def __init__(self, center, orientation):
        # W, L , H
        renault_zero_dims = [1.730, 4.084, 1.562]
        super().__init__(center=center, size=renault_zero_dims, orientation=Quaternion(orientation), name="EgoCar")

    @staticmethod
    def from_pose(ego_pose):
        return EgoBox(center=ego_pose['translation'], orientation=ego_pose['rotation'])