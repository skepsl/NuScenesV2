...
class Visualizer:
    ...
    def pc_loader(self, token):
        ...
        car_from_current = transform_matrix(current_cs['translation'], pyquaternion.Quaternion(current_cs['rotation']),
                                            inverse=False)
    ...
    def draw_polygons(self, ax, cur_polygons, scale_x, scale_y, facecolor, alpha):
        ...
        contours = np.concatenate(
            [col_pels.reshape(cur_polygons.shape[0], 1), self.map_size - row_pels.reshape(cur_polygons.shape[0], 1)],
            axis=1)
    ...
    def draw_drivable_space(self, ax, xyz, yaw, x_range, y_range, scene_location, alpha):
        ...
        for record in records:
            polygons = [self.map.nusc_maps[scene_location].extract_polygon(polygon_token) for polygon_token in
    ...
    def topview_hdmap(self, ax, lidar_now_token, scene_location, x_range, y_range, map_size, agent=None,
                      IsAgentOnly=False, BestMatchLaneOnly=False):
    ...
    def topview_trajectory(self, ax, gt, pred):
        ...
        # update, 211007
        if self.model_mode == 'vehicle':
            ax.plot(col_pels[self.obs_len - 1:], row_pels[self.obs_len - 1:], 'o-', linewidth=1.0, color=(0, 0, 0),
                    alpha=0.5)
            ax.plot(col_pels[:self.obs_len], row_pels[:self.obs_len], 'o', linewidth=1.0, color=(0.5, 0.5, 0.5),
                    alpha=0.5)
        else:
            ax.plot(col_pels[self.obs_len - 1:], row_pels[self.obs_len - 1:], '-', linewidth=1.0, color=(0, 0, 0),
                    alpha=0.5)
            ax.plot(col_pels[:self.obs_len], row_pels[:self.obs_len], '-', linewidth=1.0, color=(0.5, 0.5, 0.5),
                    alpha=0.5)
