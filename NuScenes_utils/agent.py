...
from preprocessing_utils.trajectory_filter import LinearModelIntp, AverageFilter, HoleFilling
import numpy as np
import math


class Agent:
    def __init__(self, types=None, attribute=None, track_id=None, agent_id=None, obs_len=None, pred_len=None):
        ...
        self.linearmodel = LinearModelIntp()  # Linear Interpolation
    ...
    def trajectory_curvature(self):
        ...
        if num_nan / float(seq_len) > 0.3:
            ...
            for i in range(seq_len):  # seq_len: 12
                if ~np.isnan(self.trajectory[i, 0]):  # check the first non NaN
                    first_pos = self.trajectory[i, 1:3]
                    first_idx = i
                    break

            for i in range(seq_len-1, -1, -1):   # check the last non NaN
                if ~np.isnan(self.trajectory[i, 0]):
                    last_pos = self.trajectory[i, 1:3]
                    last_idx = i
                    break

            lane_dist = np.sqrt(np.sum((last_pos - first_pos)**2))  #

            valid_trajectory = self.trajectory[first_idx:last_idx + 1, :]
            point_dist = np.sqrt(np.sum((valid_trajectory[1:, 1:3] - valid_trajectory[:-1, 1:3])**2, axis=1))  # point to next point
            nan_pos = np.isnan(point_dist)
            lane_length = np.sum(point_dist[~nan_pos])

            self.curvature = (lane_length / (lane_dist + 1E-10)) - 1.0
            self.trajectory_len = lane_length
    ...
    def update_status(self):
        ...
        elif 0.1 < self.curvature <= 0.3 and self.trajectory_len < 2:
            self.status.append('stop')
        else:
